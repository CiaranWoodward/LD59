...
# Fly the target in from the right (and make sure its visible)
static func fly_on_tween(target: Node2D, fly_time: float, delay: float = 0.0) -> Tween:
	var tween = target.create_tween()
	target.visible = true
	tween.set_trans(Tween.TRANS_BACK)
	tween.set_ease(Tween.EASE_OUT)
	var target_position = target.global_position
	target.global_position = Vector2(target.global_position.x + target.get_viewport_rect().size.x, target.global_position.y)
	tween.tween_interval(delay)
	tween.tween_property(target, "global_position", target_position, fly_time)
	return tween

# Fly the target off to the left (and hide it). Restore original position after flying off so it can be flown on again
static func fly_off_tween(target: Node2D, fly_time: float, delay: float = 0.0) -> Tween:
	var tween = target.create_tween()
	tween.set_trans(Tween.TRANS_BACK)
	tween.set_ease(Tween.EASE_IN)
	var original_position = target.global_position
	var target_position = Vector2(target.global_position.x - target.get_viewport_rect().size.x, target.global_position.y)
	tween.tween_interval(delay)
	tween.tween_property(target, "global_position", target_position, fly_time)
	# After flying off, reset position and hide
	tween.tween_property(target, "visible", false, 0.0)
	tween.tween_property(target, "global_position", original_position, 0.0)
	return tween